...
这是“小狗远程控制”的**公开接入 SDK、抽象协议和教学示例**。它用于帮助车辆开发者理解如何把自己的车辆能力接入开放平台。
...
- 小狗远程控制服务器端源码和数据库；
- WordPress 管理后台、计费、风控和审核实现；
- 正式环境密钥、用户数据或车辆会话数据；
- 官方车辆适配器、内部变量、触发器和脚本映射；
- proof、授权、动力控制或防破解实现；
- 可直接控制任意 OMSI 车辆的万能脚本。

公开 SDK 与闭源平台服务通过版本化开放接口通信。这是一种“开放 SDK + 闭源服务”的架构。
...
第三方车辆适配器（由车辆作者实现）
          │
          ▼
该车辆自己的脚本、变量或 Trigger
```

本仓库只定义中间的抽象层。每款车辆的实际变量、脚本和安全条件由该车辆开发者自行实现。

## 示例

- `examples/cpp/vehicle_adapter_example.cpp`：少量命令白名单、安全检查和抽象车辆接口。
...
示例仅用于说明思路，不是正式车辆适配器，也不包含任何官方车辆内部实现。

## 基本安全规则

- 不要把 `Project Secret` 写入公开源码、截图或日志。
- 最终用户安装包不应长期内置开发者主密钥。
